function PHsub_iter(scen_data, gap, delayMode, z_bar, w, rho, filename_Node, filename_Generator, filename_Line, filename_Storage, filename_Station, filename_SMP)
    ...
    @variable(m, PHA_2)
    if Contin == 0
        @objective(m, Min, IC + OC + PHA_1 + PHA_2 + PreventMultiSol)
    elseif Contin == 1
        @objective(m, Min, IC + EC + PHA_1 + PHA_2 + PreventMultiSol)
    end
    ...
    @constraint(m, PHA_1 == sum(sum(w.z[k,b] * z[k,b] for b in 1:N_bus) + w.x[k] * x[k] for k in 1:N_storage))
    #@constraint(m, PHA_1 == 0)
    @constraint(m, PHA_2 == sum(sum( (rho.z[k,b]/2)*(z[k,b]-z_bar[k,b])^2 for b in 1:N_bus) for k in 1:N_storage))
    ...
    penalty1 = getvalue(PHA_1)
    penalty2 = getvalue(PHA_2)
...
#z,
    return status, obj_value, penalty1, penalty2, simulation_time, result_ESlocation, result_SoC, u, z, result_v, result_a, result_pg, result_qg, result_IC, result_OC, result_EC, result_OCinter, result_OCgen, result_OCloadshed, result_OCstorage, result_x, result_sigma_d, result_sigma_l
end
